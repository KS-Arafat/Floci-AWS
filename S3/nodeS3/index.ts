...
const download_url = await getSignedUrl(
	s3,
	new GetObjectCommand({ Bucket: "nodebuck2", Key: "dump_copy.tmp" }),
	{ expiresIn: 3600 },
);
console.log("Presigned Download URL:", download_url);

const upload_url = await getSignedUrl(
	s3,
	new PutObjectCommand({ Bucket: "nodebuck1", Key: "dump_upload.tmp" }),
	{ expiresIn: 3600 },
);
console.log("Presigned Upload URL:", upload_url);
const fileUpload = fs.readFileSync("./dump.tmp");
...
if (response.ok) {
	...
	console.error("Failed to upload file using presigned URL");
}
